...
async function loadPending(found, pending, kinds, report) {
  ...
  for (let i = 0; i < wanted.length && !stopped; i++) {
    ...
    try {
      ...
      bubble = await TG.refind(entry, scroller, { deep: false });
      ...
      const src = isDoc ? TG.docStreamUrl(bubble) : await TG.openForSrc(bubble);
      ...
      // gets a result to decide on — 99-main.js discards it. Cancelled still
      // propagates: that result would mix two chats.
      if (e instanceof TG.Stopped) { stopped = true; failures.push('stopped'); break; }
      if (e instanceof TG.Cancelled) throw e;
      ...
      if (bubble && !isDoc) {
        ...
        await TG.sleep(500);   // let the viewer finish closing before the next click
      }
    }
  }
