// The media-type manifest: the checkbox list that doubles as the ledger.
//
...
// Build one manifest row. Shared by renderTypes() and updateCounts() so a row
// added mid-scan is identical to one built at the end of a pass.
function buildRow(t) {
...
export function renderTypes(types) {
  ...
  // The idle explainer is only useful before the first scan.
...
// Render any state message, whether it arrived live or via a resync on open.

// Flag the manifest as a floor rather than a total. A stopped scan counted
// only what it reached, so the header says so instead of implying the chat
// held nothing more.
...
// Update counts in place during a running scan.
//
// renderTypes() rebuilds the list, which would discard the user's checkbox
// selection on every tick. This only touches the numbers, adds rows for types
// that appear mid-scan, and leaves existing selection alone.
export function updateCounts(types) {
  if (!types?.length) return;

  // No manifest yet (the first types of the scan): build one.
  ...
  for (const t of types) {
    ...
    if (cb) {
      // Existing row: only the tally changes.
      const n = cb.parentElement.querySelector('.n');
      if (n) n.textContent = t.count;
      continue;
    }
    // A type first seen mid-scan: append it, checked like the rest.
