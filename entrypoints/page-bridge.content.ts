...
export default defineContentScript({
  ...
  main() {
    ...
    async function fetchRanged(url: string, id: string): Promise<{ buffer: ArrayBuffer; mime: string | null }> {
      const parts: ArrayBuffer[] = [];
      let offset = 0, total: number | null = null, mime: string | null = null;
    ...
    window.addEventListener('message', async ev => {
      ...
      try {
        ...
        window.postMessage({ [TAG]: 'error', id: msg.id, message: (e as Error).message }, '*');
      }
    });
  },
});
